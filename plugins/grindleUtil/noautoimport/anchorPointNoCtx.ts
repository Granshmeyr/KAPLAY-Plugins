...
export function anchorPointNoCtx(k: KAPLAYCtx) {
    ...
    return {
        bot: (obj: Params[0]): Vec2 => getBot(obj, false),
        botInWorld: (obj: Params[0]): Vec2 => getBot(obj, true),
        center: (obj: Params[0]): Vec2 => getCenter(obj, false),
        centerInWorld: (obj: Params[0]): Vec2 => getCenter(obj, true),
        topleft: (obj: Params[0]): Vec2 => getTopleft(obj, false),
        topleftInWorld: (obj: Params[0]): Vec2 => getTopleft(obj, true),
    }
    ...
    function getTopleft(
        ...
        const pos = ((): Vec2 => {
            if (a === "center")
                return o.pos.add(k.vec2(-half(w), -half(h)).scale(s))
    ...
    function getAnchorPosData(
        ...
        return {
            o: obj,
            s: obj.has("scale")
                ? (obj as unknown as GameObj<ScaleComp>).scale
                : 1,
            a: obj.has("anchor")
                ? (obj as GameObj<AnchorComp>).anchor
                : "topleft",
            w: obj.width,
            h: obj.height,
        }
